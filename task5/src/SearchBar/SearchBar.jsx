...
function SearchBar(props) {
    ...
    async function getMovies() {
        ...
        for (let movie of fetchedMovies) {
            if (favoritesMovies.some(e => e.id === movie.id)) {
    ...
    useEffect(() => {
        (async () => {
            await getMovies();
        })().catch(error => { alert("При получении фильмов произошла ошибка"); });
    }, [])
        ...
        <div className="wrapper-0">
            ...
            <div className="movie-bar-footer">
                <div className="footer-wrapper">
                    <span className="footer-text">{`Найдено ${moviesToShow && moviesToShow.length} фильмов`}</span>
